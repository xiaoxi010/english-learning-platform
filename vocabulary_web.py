...
def _serialize_word_group(group):
    if group.get('created_time'):
        group['created_time'] = str(group['created_time'])
    return group


@vocabulary_bp.route('/api/group/detail')
def get_group_detail_query():
    """获取单词组详情（查询参数，避免组名含特殊字符时路径匹配失败）"""
    dict_name = request.args.get('dict_name', '').strip()
    group_name = request.args.get('group_name', '').strip()
    if not dict_name or not group_name:
        return jsonify({'error': '参数不完整'}), 400
    vocab_manager = get_vocab_manager()
    group = vocab_manager.get_word_group(group_name, dict_name)
    if group:
        return jsonify(_serialize_word_group(group))
